...
object algorithms {
  def main(args: Array[String]) {
    ...
    val stationdata=spark.read.option("header","true").csv("D:\\BDP\\ICP_%\\Datasets\\Datasets\\201508_station_data.csv")
    val tripdata=spark.read.option("header","true").csv("D:\\BDP\\ICP_%\\Datasets\\Datasets\\201508_trip_data.csv")
    ...
    //4.Apply page rank
    val result4 = stationGraph.pageRank.resetProbability(0.15).maxIter(3).tol(0.01).run()
